...
package net.meano.playerheads;

import java.util.List;
import java.util.Random;
import net.meano.playerheads.events.MobDropHeadEvent;
import net.meano.playerheads.events.PlayerDropHeadEvent;
...
//import fr.neatmonster.nocheatplus.hooks.NCPExemptimonManager;
...
public class PlayerHeadsListener implements Listener {
	...
	public void onEntityDeath(EntityDeathEvent event) {
		...
		if (killer != null) {
			...
			if (weapon != null) {
				...
				//plugin.getLogger().info("ÇÀ¶á¸ÅÂÊ"+lootingrate);
			}
		}
	...
	public void EntityDeathHelper(EntityDeathEvent event, Enum<?> type, Double droprate) {
		...
		Double dropchance = prng.nextDouble();
		Player killer = event.getEntity().getKiller();
		//plugin.getLogger().info("µôÂä»ú»á"+dropchance + "µôÂä¸ÅÂÊ" + droprate + event.getEntityType().name());
	...
	/*
	 * public void onBlockBreak(BlockBreakEvent event) { if (event
	 * instanceof FakeBlockBreakEvent) { return; } Block block =
	 * event.getBlock(); Player player = event.getPlayer(); if
	 * ((player.getGameMode() != GameMode.CREATIVE) && (block.getType() ==
	 * Material.SKULL)) { Skull skull = (Skull) block.getState(); if
	 * (skull.hasOwner()) { CustomSkullType skullType =
	 * CustomSkullType.get(skull.getOwner()); if (skullType != null) {
	 * plugin.getServer().getPluginManager().callEvent(new
	 * PlayerAnimationEvent(player));
	 * plugin.getServer().getPluginManager().callEvent(new
	 * BlockDamageEvent(player, block, player.getItemInHand(), true));
	 * FakeBlockBreakEvent fakebreak = new FakeBlockBreakEvent(block,
	 * player); plugin.getServer().getPluginManager().callEvent(fakebreak);
	 * 
	 * if (fakebreak.isCancelled()) { event.setCancelled(true); } else {
	 * Location location = block.getLocation(); ItemStack item =
	 * Tools.Skull(skullType);
	 * 
	 * event.setCancelled(true); block.setType(Material.AIR);
	 * location.getWorld().dropItemNaturally(location, item); } } } } }
	 */
	/*
	 * @EventHandler public void onPlayerJoin(PlayerJoinEvent event) {
	 * Player player = event.getPlayer(); if
	 * (player.hasPermission("playerheads.update") &&
	 * plugin.getUpdateReady()) { Tools.formatMsg(player, Lang.UPDATE1,
	 * plugin.getUpdateName()); Tools.formatMsg(player, Lang.UPDATE3,
	 * "http://curse.com/bukkit-plugins/minecraft/" + Config.updateSlug); }
	 * }
	 */
}
