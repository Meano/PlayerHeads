...
public class Updater {
	...
	private String updateFolder;// The folder that downloads will be placed
					// in
	...
	public enum UpdateResult {
		/**
		 * The updater found an update, and has readied it to be loaded
		 * the next time the server restarts/reloads.
		 */
		SUCCESS,
		/**
		 ...
		 * contain the a version in the format 'vVersion' such as
		 * 'v1.0'.
		 */
		FAIL_NOVERSION,
		/**
		 * The id provided by the plugin running the updater was invalid
		 * and doesn't exist on DBO.
		 */
		FAIL_BADID,
		/**
		 * The server administrator has improperly configured their API
		 * key in the configuration
		 */
		FAIL_APIKEY,
		/**
		 * The updater found an update, but because of the UpdateType
		 * being set to NO_DOWNLOAD, it wasn't downloaded.
		 */
		UPDATE_AVAILABLE
	}
	...
	public enum UpdateType {
		/**
		 * Run a version check, and then if the file is out of date,
		...
		NO_VERSION_CHECK,
		/**
		 * Get information about the version and the download size, but
	 ...
	 * @param file
	 *                The file that the plugin is running from, get this by
	 *                doing this.getFile() from within your main class.
	 * @param type
	 *                Specify the type of update this will be. See
	 *                {@link UpdateType}
	 * @param announce
	 *                True if the program should announce the progress of
	 *                new updates in console
	 */
	 ...
	 * it is necessary to wait for the thread to finish before allowing
	...
	private void saveFile(File folder, String file, String u) {
		...
		try {
			...
			// Just a quick check to make sure we didn't leave any
	...
	private void unzip(String file) {
		try {
			...
			// Move any plugin data folders that were included to
			// the right place, Bukkit won't do this for us.
				...
				if (dFile.isDirectory()) {
					if (this.pluginFile(dFile.getName())) {
						final File oFile = new File(this.plugin.getDataFolder().getParent(), dFile.getName()); // Get
																	// current
																	// dir
						final File[] contents = oFile.listFiles(); // List
												// of
												// existing
												// files
												// in
						...
						{
							boolean found = false;
							for (final File xFile : contents) // Loop
												// through
												// contents
												// to
												// see
		...
		if (this.type != UpdateType.NO_VERSION_CHECK) {
			...
			if (title.split(" v").length == 2) {
				...
				if (this.hasTag(version) || version.split("-")[0].equalsIgnoreCase(remoteVersion.split("-")[0])) {
					...
					// or this build is tagged for no-update
					this.result = Updater.UpdateResult.NO_UPDATE;
					return false;
				}
			} else {
				// The file's name did not contain the string
				// 'vVersion'
