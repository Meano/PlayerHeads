...
public final class PlayerHeads extends JavaPlugin {
	...
	/*
	 * private void initUpdater() { try { if
	 * (configFile.getBoolean("updatecheck") && !(updateReady)) { Updater
	 * updater = new Updater(this, Config.updateID, this.getFile(),
	 * Updater.UpdateType.NO_DOWNLOAD, false); // Start Updater but just do
	 * a version check updateReady = updater.getResult() ==
	 * Updater.UpdateResult.UPDATE_AVAILABLE; // Determine if there is an
	 * update ready for us updateName = updater.getLatestName(); // Get the
	 * latest version } } catch (Exception e) {
	...
	/*
	 * public boolean getUpdateReady() { return updateReady; }
	 * 
